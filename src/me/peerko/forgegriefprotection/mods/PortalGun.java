...
public class PortalGun extends ProtBase {
    ...
    public boolean update(Entity e) throws Exception {
        ...
        MovingObjectPosition collision = // e.worldObj.clip(currPos,nextPos);
        	e.worldObj.rayTraceBlocks_do_do(currPos, nextPos, false, false);
        ...
        if (owner != null && !systemOwnerNames.contains(owner)) {
            ...
            EntityPlayer r = MinecraftServer.getServer().getConfigurationManager().getPlayerForUsername(owner);
            //Resident r = ProtectionEvents.instance.lastOwner = MyTownDatasource.instance.getResident(owner);
            if (r == null /*|| !r.isOnline()*/) {
                return false; //"Owner " + owner + " not found or offline";
            }
            ...
            for (int i = 0; i < 7; i++) {
                ...
                if (!canBuild(r, collision.blockX + xPos[i], collision.blockY
                        + yPos[i], collision.blockZ + zPos[i]))
                	return false;
            }
        } else {
        	// what are "default portals" ?
        	return false;
